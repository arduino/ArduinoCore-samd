...
int I2SClass::begin(int mode, long sampleRate, int bitsPerSample, bool driveClock)
{
  ...
  if (driveClock) {
    // set up clock
#if defined(CLOCKCONFIG_CLOCK_SOURCE) && ((CLOCKCONFIG_CLOCK_SOURCE == CLOCKCONFIG_32768HZ_CRYSTAL) || (CLOCKCONFIG_CLOCK_SOURCE == CLOCKCONFIG_HS_CRYSTAL))
    enableClock(sampleRate * bitsPerSample);
#else
...
#if defined(CLOCKCONFIG_CLOCK_SOURCE) && ((CLOCKCONFIG_CLOCK_SOURCE == CLOCKCONFIG_32768HZ_CRYSTAL) || (CLOCKCONFIG_CLOCK_SOURCE == CLOCKCONFIG_HS_CRYSTAL))
  GCLK->GENCTRL.bit.SRC = GCLK_GENCTRL_SRC_DPLL96M_Val;
#else
